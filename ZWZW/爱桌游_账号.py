...
# 运行：python 爱桌游_账号.py   （鉴权信息见下方内置配置；环境变量 ZHUOYOUX_ACCOUNT / ZHUOYOUX_DOMAIN / ZHUOYOUX_PROXY / ZHUOYOUX_DELAY / ZHUOYOUX_COOKIE_CACHE / ZHUOYOUX_FORCE_LOGIN / ZHUOYOUX_TLS_IMPERSONATE 优先）
# 多账户：环境变量或内置配置里填多个，用 换行 / @@ / && 分隔，也可直接写 Python 列表
# 账号/密码鉴权：登录后把会话 Cookie（及登录步骤提取的变量）缓存到脚本同目录 <名>.cookie_cache.json，下次直接复用、过期自动重新登录（ZHUOYOUX_COOKIE_CACHE 可改路径）。若站点不靠 Cookie 维持登录（纯 token / 每次需重登），设 ZHUOYOUX_FORCE_LOGIN=1 强制每次账号密码登录、不读写缓存。
...
BUILTIN_ACCOUNT = ""  # 推荐：账号#密码（# 也可写 :）；多组换行/@@/&&分隔，如 "u1#p1@@u2#p2"；也可用 ZHUOYOUX_ACCOUNT 环境变量
BUILTIN_USER = "xxx"  # 登录账号；多账户用换行/@@/&&分隔，或写列表
BUILTIN_PASS = "xxx"  # 登录密码，与账号一一对应（只填一个则多账号共用）
BUILTIN_FORCE_LOGIN = False  # 设为 True 强制每次账号密码登录（不读写 cookie 缓存），应对不靠 cookie 维持登录的站点；也可用 ZHUOYOUX_FORCE_LOGIN=1 开启
...
BUILTIN_AUTO_INSTALL = True  # 依赖缺失时自动 pip install（如 curl_cffi）；设 ZHUOYOUX_AUTO_INSTALL=0 关闭
BUILTIN_STRICT_EMPTY = True  # 汇总变量全空时判失败（防"未登录却报成功"）；站点本就不返回数据可设 ZHUOYOUX_STRICT_EMPTY=0
# =================================

BASE = "https://www.zhuoyoux.com"

ENTRIES = [{'request': {'method': 'POST',
              'url': 'https://www.zhuoyoux.com/lzy/login',
              'headers': [{'name': 'User-Agent', 'value': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:130.0) Gecko/20100101 Firefox/130.0'},
                          {'name': 'Accept', 'value': 'application/json, text/plain, */*'},
                          {'name': 'Accept-Language', 'value': 'zh-CN,zh;q=0.8,zh-TW;q=0.7,zh-HK;q=0.5,en-US;q=0.3,en;q=0.2'},
                          {'name': 'Accept-Encoding', 'value': 'gzip, deflate, br, zstd'},
                          {'name': 'Content-Type', 'value': 'application/json;charset=UTF-8'},
                          {'name': 'ywj', 'value': 'ywj'},
                          {'name': 'Origin', 'value': 'https://www.zhuoyoux.com'},
                          {'name': 'DNT', 'value': '1'},
                          {'name': 'Sec-GPC', 'value': '1'},
                          {'name': 'Connection', 'value': 'keep-alive'},
                          {'name': 'Referer', 'value': 'https://www.zhuoyoux.com/'},
 ...
 {'request': {'method': 'GET',
              ...
              'headers': [{'name': 'User-Agent', 'value': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:130.0) Gecko/20100101 Firefox/130.0'},
                          ...
                          {'name': 'Cache-Control', 'value': 'no-cache'}],
              'cookies': []},
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}, {'re': '您已经签到过了', 'from': 'content'}],
           'failed_asserts': [],
           'extract_variables': [{'name': 'qd_back', 're': 'data":"(.*?)"', 'from': 'content'}]}},
 {'request': {'method': 'GET',
              ...
              'headers': [{'name': 'User-Agent', 'value': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:130.0) Gecko/20100101 Firefox/130.0'},
                          ...
                          {'name': 'Sec-GPC', 'value': '1'},
                          {'name': 'Connection', 'value': 'keep-alive'},
                          {'name': 'Referer', 'value': 'https://www.zhuoyoux.com/user?id=1'},
                          {'name': 'Sec-Fetch-Dest', 'value': 'empty'},
                          {'name': 'Sec-Fetch-Mode', 'value': 'cors'},
                          {'name': 'Sec-Fetch-Site', 'value': 'same-origin'},
                          {'name': 'Priority', 'value': 'u=0'},
  ...
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}],
           'failed_asserts': [{'re': '403', 'from': 'status'}],
           'extract_variables': [{'name': 'money', 're': 'money":"(.*?)"', 'from': 'content'},
                                 {'name': 'nickname', 're': 'nickname":"(.*?)"', 'from': 'content'},
                                 {'name': 'email', 're': 'email":"(.*?)"', 'from': 'content'}]}},
 {'comment': 'Unicode转换',
  'request': {'method': 'POST',
