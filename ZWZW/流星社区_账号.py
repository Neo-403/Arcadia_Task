...
# 运行：python 流星社区_账号.py   （鉴权信息见下方内置配置；环境变量 LIUXINGW_ACCOUNT / LIUXINGW_DOMAIN / LIUXINGW_PROXY / LIUXINGW_DELAY / LIUXINGW_COOKIE_CACHE / LIUXINGW_FORCE_LOGIN / LIUXINGW_TLS_IMPERSONATE 优先）
# 多账户：环境变量或内置配置里填多个，用 换行 / @@ / && 分隔，也可直接写 Python 列表
# 账号/密码鉴权：登录后把会话 Cookie（及登录步骤提取的变量）缓存到脚本同目录 <名>.cookie_cache.json，下次直接复用、过期自动重新登录（LIUXINGW_COOKIE_CACHE 可改路径）。若站点不靠 Cookie 维持登录（纯 token / 每次需重登），设 LIUXINGW_FORCE_LOGIN=1 强制每次账号密码登录、不读写缓存。
...
BUILTIN_ACCOUNT = ""  # 推荐：账号#密码（# 也可写 :）；多组换行/@@/&&分隔，如 "u1#p1@@u2#p2"；也可用 LIUXINGW_ACCOUNT 环境变量
BUILTIN_USER = "xxx"  # 登录账号；多账户用换行/@@/&&分隔，或写列表
BUILTIN_PASS = "xxx"  # 登录密码，与账号一一对应（只填一个则多账号共用）
BUILTIN_FORCE_LOGIN = False  # 设为 True 强制每次账号密码登录（不读写 cookie 缓存），应对不靠 cookie 维持登录的站点；也可用 LIUXINGW_FORCE_LOGIN=1 开启
...
BUILTIN_AUTO_INSTALL = True  # 依赖缺失时自动 pip install（如 curl_cffi）；设 LIUXINGW_AUTO_INSTALL=0 关闭
BUILTIN_STRICT_EMPTY = True  # 汇总变量全空时判失败（防"未登录却报成功"）；站点本就不返回数据可设 LIUXINGW_STRICT_EMPTY=0
# =================================

BASE = "https://bbs.liuxingw.com"
...
ENTRIES = [{'request': {'method': 'POST',
 ...
 {'request': {'method': 'POST',
              ...
              'headers': [{'name': 'Accept', 'value': 'application/json, text/javascript, */*; q=0.01'},
                          ...
                          {'name': 'DNT', 'value': '1'},
                          {'name': 'Origin', 'value': 'https://bbs.liuxingw.com'},
                          {'name': 'Referer', 'value': 'https://bbs.liuxingw.com/'},
                          {'name': 'Sec-Fetch-Dest', 'value': 'empty'},
                          {'name': 'Sec-Fetch-Mode', 'value': 'cors'},
                          {'name': 'Sec-Fetch-Site', 'value': 'same-origin'},
                          {'name': 'User-Agent', 'value': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Safari/537.36 Edg/123.0.0.0'},
                          {'name': 'X-Requested-With', 'value': 'XMLHttpRequest'},
                          {'name': 'sec-ch-ua', 'value': '"Microsoft Edge";v="123", "Not:A-Brand";v="8", "Chromium";v="123"'},
                          {'name': 'sec-ch-ua-mobile', 'value': '?0'},
                          {'name': 'sec-ch-ua-platform', 'value': '"Windows"'}],
              'cookies': []},
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}], 'failed_asserts': [], 'extract_variables': [{'name': 'info', 're': 'info":"(.+)"', 'from': 'content'}]}},
 {'comment': 'Unicode转换',
  'request': {'method': 'POST', 'url': 'api://util/unicode', 'headers': [], 'cookies': [], 'data': 'html_unescape=false&content={{info}}'},
