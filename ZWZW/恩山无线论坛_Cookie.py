...
# 运行：python 恩山无线论坛_Cookie.py   （鉴权信息见下方内置配置；环境变量 COM_COOKIE / COM_PROXY / COM_DELAY / COM_TLS_IMPERSONATE 优先）
# 多账户：账号用 <SITE_TAG>_ACCOUNT 一次填完（"账号#密码"，多组换行 / @@ / && 分隔）；Cookie 用 换行 / @@ / && 分隔，也可直接写 Python 列表
# Cookie 鉴权（自带 Cookie，不本地存储；Cookie 失效请更新 BUILTIN_COOKIE / COM_COOKIE）
...
BUILTIN_AUTO_INSTALL = True  # 依赖缺失时自动 pip install（如 curl_cffi）；设 COM_AUTO_INSTALL=0 关闭
BUILTIN_STRICT_EMPTY = True  # 汇总变量全空时判失败（防"未登录却报成功"）；站点本就不返回数据可设 COM_STRICT_EMPTY=0
# =================================

BASE = "https://www.right.com.cn"
...
ENTRIES = [{'comment': '每日登录',
  'request': {'method': 'GET', 'url': 'https://www.right.com.cn/forum/forum.php', 'headers': [{'name': 'Cookie', 'value': '{{cookie}}'}], 'cookies': []},
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}],
           'failed_asserts': [],
           'extract_variables': [{'name': 'formhash', 're': 'name="formhash"\\s+value="([^"]+)"', 'from': 'content'}, {'name': 'uid', 're': 'space-uid-(\\d+)', 'from': 'content'}]}},
 {'request': {'method': 'POST',
 ...
 {'comment': '',
  'request': {'method': 'GET',
              'url': 'https://www.right.com.cn/forum/home.php?mod=space&uid={{uid|urlencode}}&do=profile&mycenter=1',
              'headers': [{'name': 'Cookie', 'value': '{{cookie}}'}],
              'cookies': []},
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}],
           'failed_asserts': [],
           'extract_variables': [{'name': 'esb', 're': '恩山币<\\/em>\\s*(\\d+)', 'from': 'content'},
                                 {'name': 'user', 're': '<h2[^>]*>\\s*([^<]+)', 'from': 'content'},
                                 {'name': 'gid', 're': '用户组[^>]*>.*?<a[^>]*>([^<]+)</a>', 'from': 'content'}]}},
 {'comment': 'Unicode转换',
  'request': {'method': 'POST', 'url': 'api://util/unicode', 'headers': [], 'cookies': [], 'data': 'content=用户名：{{user}}，uid：{{uid}}，用户组：{{gid}}，恩山币：{{esb}}'},
