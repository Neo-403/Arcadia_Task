...
# 运行：python 单机Game怀旧馆_账号.py   （鉴权信息见下方内置配置；环境变量 GAME_ACCOUNT / GAME_DOMAIN / GAME_PROXY / GAME_DELAY / GAME_COOKIE_CACHE / GAME_FORCE_LOGIN / GAME_TLS_IMPERSONATE 优先）
# 多账户：环境变量或内置配置里填多个，用 换行 / @@ / && 分隔，也可直接写 Python 列表
# 账号/密码鉴权：登录后把会话 Cookie（及登录步骤提取的变量）缓存到脚本同目录 <名>.cookie_cache.json，下次直接复用、过期自动重新登录（GAME_COOKIE_CACHE 可改路径）。若站点不靠 Cookie 维持登录（纯 token / 每次需重登），设 GAME_FORCE_LOGIN=1 强制每次账号密码登录、不读写缓存。
...
BUILTIN_ACCOUNT = ""  # 推荐：账号#密码（# 也可写 :）；多组换行/@@/&&分隔，如 "u1#p1@@u2#p2"；也可用 GAME_ACCOUNT 环境变量
BUILTIN_USER = "xxx"  # 登录账号；多账户用换行/@@/&&分隔，或写列表
BUILTIN_PASS = "xxx"  # 登录密码，与账号一一对应（只填一个则多账号共用）
BUILTIN_FORCE_LOGIN = False  # 设为 True 强制每次账号密码登录（不读写 cookie 缓存），应对不靠 cookie 维持登录的站点；也可用 GAME_FORCE_LOGIN=1 开启
BUILTIN_DOMAIN = "www.huixlife.com"  # 站点域名，如 bbs.example.com（不带 https://）；多个按账户顺序配对
...
BUILTIN_AUTO_INSTALL = True  # 依赖缺失时自动 pip install（如 curl_cffi）；设 GAME_AUTO_INSTALL=0 关闭
BUILTIN_STRICT_EMPTY = True  # 汇总变量全空时判失败（防"未登录却报成功"）；站点本就不返回数据可设 GAME_STRICT_EMPTY=0
# =================================
...
ENTRIES = [{'comment': '获取nonce',
  'request': {'method': 'GET',
              'url': 'https://{{DOMAIN}}/',
              'headers': [{'name': 'Host', 'value': '{{DOMAIN}}'},
              ...
              'data': '',
              'mimeType': ''},
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}], 'failed_asserts': [], 'extract_variables': [{'name': 'nonce', 're': '"ajax_nonce":"(.*?)"', 'from': 'content'}]}},
 {'comment': '登录',
  'request': {'method': 'POST',
  ...
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}, {'re': '登录成功', 'from': 'content'}], 'failed_asserts': [{'re': '用户名或密码不正确', 'from': 'content'}], 'extract_variables': []},
  'login': True},
 {'comment': '获取nonce，用户名',
  'request': {'method': 'GET',
  ...
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}],
           'failed_asserts': [],
           'extract_variables': [{'name': 'nonce1', 're': '"ajax_nonce":"(.*?)"', 'from': 'content'}, {'name': 'yhm', 're': '<span class="ms-2 d-none d-md-block">(.+)</span>', 'from': 'content'}]}},
 {'comment': '签到',
  'request': {'method': 'POST',
              'url': 'https://{{DOMAIN}}/wp-admin/admin-ajax.php',
              'headers': [{'name': 'Host', 'value': '{{DOMAIN}}'},
 ...
 {'comment': '获取余额',
  ...
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}],
           'failed_asserts': [],
           'extract_variables': [{'name': 'zhye', 're': '<div class="mb-2"><i class="fas fa-coins me-1"></i>(.*?)</div>', 'from': 'content'}]}},
 {'comment': 'Unicode转换',
  'request': {'method': 'POST', 'url': 'api://util/unicode', 'headers': [], 'cookies': [], 'data': 'html_unescape=false&content=用户名：{{yhm}}\\r\\n签到状态：{{msg}}\\r\\n账号余额：{{zhye}}金币'},
