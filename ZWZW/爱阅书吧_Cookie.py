...
# 运行：python 爱阅书吧_Cookie.py   （鉴权信息见下方内置配置；环境变量 IIISHU_COOKIE / IIISHU_PROXY / IIISHU_DELAY / IIISHU_TLS_IMPERSONATE 优先）
# 多账户：环境变量或内置配置里填多个，用 换行 / @@ / && 分隔，也可直接写 Python 列表
# Cookie 鉴权（自带 Cookie，不本地存储；Cookie 失效请更新 BUILTIN_COOKIE / IIISHU_COOKIE）
import os, re, sys, ssl, json, gzip, zlib, html, time, random, subprocess, urllib.request, urllib.parse, urllib.error, http.client
...
BUILTIN_COOKIE = "xxx"  # 会话 Cookie；多账户用换行/@@/&&分隔，或写列表 ["ck1","ck2"]
...
BUILTIN_AUTO_INSTALL = True  # 依赖缺失时自动 pip install（如 curl_cffi）；设 IIISHU_AUTO_INSTALL=0 关闭
BUILTIN_STRICT_EMPTY = True  # 汇总变量全空时判失败（防"未登录却报成功"）；站点本就不返回数据可设 IIISHU_STRICT_EMPTY=0
# =================================

BASE = "https://www.iiishu.com"

ENTRIES = [{'comment': '首页',
  'request': {'method': 'GET',
              'url': 'https://www.iiishu.com/',
              'headers': [{'name': 'Host', 'value': 'www.iiishu.com'},
                          {'name': 'Connection', 'value': 'keep-alive'},
                          {'name': 'Cache-Control', 'value': 'max-age=0'},
                          {'name': 'sec-ch-ua', 'value': '"Chromium";v="122", "Not(A:Brand";v="24", "Google Chrome";v="122"'},
                          {'name': 'sec-ch-ua-mobile', 'value': '?0'},
                          {'name': 'sec-ch-ua-platform', 'value': '"Windows"'},
                          {'name': 'Upgrade-Insecure-Requests', 'value': '1'},
                          {'name': 'User-Agent', 'value': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.6261.95 Safari/537.36'},
                          ...
                          {'name': 'Referer', 'value': 'https://www.iiishu.com/home.php?mod=space&do=notice&view=system'},
 ...
 {'comment': '获取formhash',
  'request': {'method': 'GET',
              'url': 'https://www.iiishu.com/k_misign-sign.html',
              'headers': [{'name': 'Host', 'value': 'www.iiishu.com'},
                          {'name': 'Connection', 'value': 'keep-alive'},
                          {'name': 'sec-ch-ua', 'value': '"Chromium";v="122", "Not(A:Brand";v="24", "Google Chrome";v="122"'},
                          {'name': 'sec-ch-ua-mobile', 'value': '?0'},
                          ...
                          {'name': 'Sec-Fetch-User', 'value': '?1'},
                          {'name': 'Sec-Fetch-Dest', 'value': 'document'},
                          {'name': 'Referer', 'value': 'https://www.iiishu.com/'},
                          {'name': 'Accept-Encoding', 'value': 'gzip, deflate, br'},
                          {'name': 'Accept-Language', 'value': 'zh-CN,zh;q=0.9'},
                          {'name': 'Cookie', 'value': '{{cookie}}'}],
              'cookies': [],
              'data': '',
              'mimeType': ''},
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}], 'failed_asserts': [], 'extract_variables': [{'name': 'formhash', 're': 'name="formhash" value="(.+)"', 'from': 'content'}]}},
 {'comment': '签到',
  'request': {'method': 'GET',
              'url': 'https://www.iiishu.com/k_misign-sign.html?operation=qiandao&format=global_usernav_extra&formhash={{formhash|urlencode}}&inajax=1&ajaxtarget=k_misign_topb',
              'headers': [{'name': 'Host', 'value': 'www.iiishu.com'},
                          {'name': 'Connection', 'value': 'keep-alive'},
                          {'name': 'sec-ch-ua', 'value': '"Chromium";v="122", "Not(A:Brand";v="24", "Google Chrome";v="122"'},
              ...
              'data': '',
              'mimeType': ''},
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}], 'failed_asserts': [{'re': '您当前的访问请求当中含有非法字符，已经被系统拒绝', 'from': 'content'}], 'extract_variables': []}},
 {'comment': '签到页',
  'request': {'method': 'GET',
              'url': 'https://www.iiishu.com/k_misign-sign.html',
              'headers': [{'name': 'Host', 'value': 'www.iiishu.com'},
  ...
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}],
           'failed_asserts': [{'re': '立即注册', 'from': 'content'}],
           'extract_variables': [{'name': 'lxqd', 're': '<input type="hidden" class="hidnum" id="lxdays" value="(\\d+)', 'from': 'content'},
                                 {'name': 'qddj', 're': '<input type="hidden" class="hidnum" id="lxlevel" value="(\\d+)', 'from': 'content'},
                                 {'name': 'jfjl', 're': '<input type="hidden" class="hidnum" id="lxreward" value="(\\d+)', 'from': 'content'},
                                 {'name': 'zts', 're': '<input type="hidden" class="hidnum" id="lxtdays" value="(\\d+)', 'from': 'content'}]}},
 {'comment': '积分页',
  'request': {'method': 'GET',
              'url': 'https://www.iiishu.com/home.php?mod=spacecp&ac=credit&showcredit=1',
              'headers': [{'name': 'Host', 'value': 'www.iiishu.com'},
                          {'name': 'Connection', 'value': 'keep-alive'},
                          {'name': 'Cache-Control', 'value': 'max-age=0'},
                          {'name': 'sec-ch-ua', 'value': '"Chromium";v="122", "Not(A:Brand";v="24", "Google Chrome";v="122"'},
              ...
              'mimeType': ''},
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}],
           'failed_asserts': [{'re': '您需要先登录才能继续本操作', 'from': 'content'}],
           'extract_variables': [{'name': 'jb', 're': '<em> 金币: </em>(\\d+)', 'from': 'content'},
                                 {'name': 'yb', 're': '<li><em> 银币: </em>(\\d+)', 'from': 'content'},
                                 {'name': 'jf', 're': '<li class="cl"><em>积分: </em>(\\d+)', 'from': 'content'}]}},
 {'comment': 'Unicode转换',
  'request': {'method': 'POST',
              'url': 'api://util/unicode',
              'headers': [],
              'cookies': [],
              'data': 'html_unescape=false&content=连续签到：{{lxqd}}天，签到等级：Lv.{{qddj}}，积分奖励：{{jfjl}}，总天数：{{zts}}天。\\r\\n金币：{{jb}}，银币：{{yb}}，积分：{{jf}}。'},
...
def _eval_val(expr, ctx):
    ...
    if m:
        ...
        return str(val) if m.group(1) == "unicode" else val
    return ctx.get(expr, "")
...
def run_one(proxy, ctx, use_cookiejar, skip_login=False, capture_vars=None, session=None):
    ...
    def _cross_domain(u):
        # 仅当请求主机与 BASE 属于“完全不同站点”时才视为跨域（剔除整页 HAR 的第三方资源）。
        # 同注册域子域（api.x.com 之于 x.com、www.x.com、cdn.x.com）视为本站，不跳过——
        # 否则“签到/积分接口挂在 api. 子域”的站点会被误杀，变量提取为空(解析不全)。
        if not BASE:
            return False
        h, b = _host(u), _host(BASE)
    ...
    for e in ENTRIES:
        ...
        if url.startswith("http") and (_is_static(url) or _cross_domain(url)):
            continue
...
def run():
    ...
    cookies = _split_multi(_env("COOKIE", BUILTIN_COOKIE))
    if not cookies:
    ...
    for i, ck in enumerate(cookies):
        ...
        accounts.append({"ctx": c, "jar": False, "secrets": [ck],
                         "label": (ck[:10] + "...") if len(ck) > 10 else ck})
