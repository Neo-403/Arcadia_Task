...
# 运行：python 父子被窝_账号.py   （鉴权信息见下方内置配置；环境变量 FZBW_ACCOUNT / FZBW_DOMAIN / FZBW_PROXY / FZBW_DELAY / FZBW_COOKIE_CACHE / FZBW_FORCE_LOGIN / FZBW_TLS_IMPERSONATE 优先）
# 多账户：环境变量或内置配置里填多个，用 换行 / @@ / && 分隔，也可直接写 Python 列表
# 账号/密码鉴权：登录后把会话 Cookie（及登录步骤提取的变量）缓存到脚本同目录 <名>.cookie_cache.json，下次直接复用、过期自动重新登录（FZBW_COOKIE_CACHE 可改路径）。若站点不靠 Cookie 维持登录（纯 token / 每次需重登），设 FZBW_FORCE_LOGIN=1 强制每次账号密码登录、不读写缓存。
...
BUILTIN_ACCOUNT = ""  # 推荐：账号#密码（# 也可写 :）；多组换行/@@/&&分隔，如 "u1#p1@@u2#p2"；也可用 FZBW_ACCOUNT 环境变量
BUILTIN_USER = "xxx"  # 登录账号；多账户用换行/@@/&&分隔，或写列表
BUILTIN_PASS = "xxx"  # 登录密码，与账号一一对应（只填一个则多账号共用）
BUILTIN_FORCE_LOGIN = False  # 设为 True 强制每次账号密码登录（不读写 cookie 缓存），应对不靠 cookie 维持登录的站点；也可用 FZBW_FORCE_LOGIN=1 开启
...
BUILTIN_AUTO_INSTALL = True  # 依赖缺失时自动 pip install（如 curl_cffi）；设 FZBW_AUTO_INSTALL=0 关闭
BUILTIN_STRICT_EMPTY = True  # 汇总变量全空时判失败（防"未登录却报成功"）；站点本就不返回数据可设 FZBW_STRICT_EMPTY=0
# =================================

BASE = "https://www.fzbw.cn"
...
ENTRIES = [{'request': {'method': 'POST',
              ...
              'headers': [{'name': 'Host', 'value': 'www.fzbw.cn'},
                          ...
                          {'name': 'User-Agent', 'value': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/119.0.6045.160 Safari/537.36'},
                          {'name': 'sec-ch-ua-platform', 'value': '"Windows"'},
                          {'name': 'Origin', 'value': 'https://www.fzbw.cn'},
                          {'name': 'Sec-Fetch-Site', 'value': 'same-origin'},
                          {'name': 'Sec-Fetch-Mode', 'value': 'cors'},
                          {'name': 'Sec-Fetch-Dest', 'value': 'empty'},
                          {'name': 'Referer', 'value': 'https://www.fzbw.cn/'},
  ...
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}], 'failed_asserts': [], 'extract_variables': [{'name': 'token', 're': 'token":"(.+)",', 'from': 'content'}]},
  'login': True},
 {'request': {'method': 'POST',
  ...
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}],
           'failed_asserts': [{'re': '请登录|未登录|失效|过期|expired|invalid|jwt_auth|"status":401|"code":401|error', 'from': 'content'}],
           'extract_variables': []}},
 {'comment': '获取积分',
  'request': {'method': 'POST',
              'url': 'https://www.fzbw.cn/wp-json/b2/v1/getUserMission',
              'headers': [{'name': 'Host', 'value': 'www.fzbw.cn'},
                          {'name': 'Connection', 'value': 'keep-alive'},
                          {'name': 'sec-ch-ua', 'value': '"Chromium";v="119", "Not?A_Brand";v="24"'},
                          {'name': 'Accept', 'value': 'application/json, text/plain, */*'},
  ...
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}],
           'failed_asserts': [{'re': '请登录|未登录|失效|过期|expired|invalid|jwt_auth|"status":401|"code":401|error', 'from': 'content'}],
           'extract_variables': [{'name': 'jj', 're': '"credit":"(\\d+)"', 'from': 'content'}, {'name': 'j', 're': '"my_credit":"(\\d+)"', 'from': 'content'}]}},
 {'comment': 'Unicode转换',
  'request': {'method': 'POST', 'url': 'api://util/unicode', 'headers': [], 'cookies': [], 'data': 'html_unescape=false&content=恭喜！您今天获得了{{jj}}积分 | 我的积分:{{j}}'},
