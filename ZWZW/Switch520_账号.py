...
# 运行：python Switch520_账号.py   （鉴权信息见下方内置配置；环境变量 SWITCH520_ACCOUNT / SWITCH520_DOMAIN / SWITCH520_PROXY / SWITCH520_DELAY / SWITCH520_COOKIE_CACHE / SWITCH520_FORCE_LOGIN / SWITCH520_TLS_IMPERSONATE 优先）
# 多账户：环境变量或内置配置里填多个，用 换行 / @@ / && 分隔，也可直接写 Python 列表
# 账号/密码鉴权：登录后把会话 Cookie（及登录步骤提取的变量）缓存到脚本同目录 <名>.cookie_cache.json，下次直接复用、过期自动重新登录（SWITCH520_COOKIE_CACHE 可改路径）。若站点不靠 Cookie 维持登录（纯 token / 每次需重登），设 SWITCH520_FORCE_LOGIN=1 强制每次账号密码登录、不读写缓存。
...
BUILTIN_ACCOUNT = ""  # 推荐：账号#密码（# 也可写 :）；多组换行/@@/&&分隔，如 "u1#p1@@u2#p2"；也可用 SWITCH520_ACCOUNT 环境变量
BUILTIN_USER = "xxx"  # 登录账号；多账户用换行/@@/&&分隔，或写列表
BUILTIN_PASS = "xxx"  # 登录密码，与账号一一对应（只填一个则多账号共用）
BUILTIN_FORCE_LOGIN = False  # 设为 True 强制每次账号密码登录（不读写 cookie 缓存），应对不靠 cookie 维持登录的站点；也可用 SWITCH520_FORCE_LOGIN=1 开启
...
BUILTIN_AUTO_INSTALL = True  # 依赖缺失时自动 pip install（如 curl_cffi）；设 SWITCH520_AUTO_INSTALL=0 关闭
BUILTIN_STRICT_EMPTY = True  # 汇总变量全空时判失败（防"未登录却报成功"）；站点本就不返回数据可设 SWITCH520_STRICT_EMPTY=0
# =================================

BASE = "https://www.switch520.org"
BUILTIN_ALLOW_CROSS = False   # 允许跨域请求(保留第三方授权/API域名步骤)：True/False；也可用 <SITE_TAG>_ALLOW_CROSS=1 开启
ALLOW_CROSS = str(_env("ALLOW_CROSS", BUILTIN_ALLOW_CROSS)).strip().lower() in ("1","true","yes","y","on")

ENTRIES = [{'comment': '登录',
  'request': {'method': 'POST',
              'url': 'https://www.switch520.org/wp-admin/admin-ajax.php',
              'headers': [{'name': 'Connection', 'value': 'keep-alive'},
                          {'name': 'Accept', 'value': '*/*'},
                          {'name': 'Content-Type', 'value': 'application/x-www-form-urlencoded; charset=UTF-8'},
                          {'name': 'X-Requested-With', 'value': 'XMLHttpRequest'},
                          {'name': 'User-Agent', 'value': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.45 Safari/537.36'},
                          {'name': 'Origin', 'value': 'https://www.switch520.org'},
                          {'name': 'Referer', 'value': 'https://www.switch520.org/'},
  ...
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}, {'re': '登录成功', 'from': 'content'}],
           'failed_asserts': [{'re': '用户名或密码错误', 'from': 'content'}, {'re': '"status":"0"', 'from': 'content'}],
           'extract_variables': []},
  'login': True},
 {'comment': '签到',
  'request': {'method': 'POST',
              ...
              'mimeType': 'application/x-www-form-urlencoded'},
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}], 'failed_asserts': [{'re': '请登录|未登录', 'from': 'content'}], 'extract_variables': [{'name': 'msg', 're': '"msg":"(.+?)"', 'from': 'content'}]}},
 {'comment': '用户页，获取余额信息',
  'request': {'method': 'GET',
              'url': 'https://www.switch520.org/user?action=index',
              'headers': [{'name': 'Connection', 'value': 'keep-alive'},
                          ...
                          {'name': 'Referer', 'value': 'https://www.switch520.org/user?action=vip'},
                          {'name': 'Accept-Encoding', 'value': 'gzip, deflate, br'},
                          {'name': 'Accept-Language', 'value': 'zh-CN,zh;q=0.9,en;q=0.8'}],
              'cookies': [],
              'data': '',
  ...
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}],
           'failed_asserts': [{'re': '请登录|未登录', 'from': 'content'}],
           'extract_variables': [{'name': 'dqyy', 're': '当前余额[:：]?\s*([\d.]+)', 'from': 'content'},
                                 {'name': 'yxf', 're': '累计消费[:：]?\s*([\d.]+)', 'from': 'content'},
                                 {'name': 'nickName', 're': '<small class="mb-0 text-muted">(.+?)<', 'from': 'content'}]}},
 {'comment': '字符串替换',
  'request': {'method': 'GET',
              'url': "api://util/string/replace?r=text&t=%C2%A0&p=%C2%A0&s=%E7%AD%BE%E5%88%B0%E4%BF%A1%E6%81%AF%EF%BC%9A%E3%80%90{{unicode(msg|default('签到成功'))|urlencode}}%E3%80%91%5Cr%5Cn%E7%94%A8%E6%88%B7%EF%BC%9A%E3%80%90{{nickName|urlencode}}%E3%80%91%20---%20%E5%BD%93%E5%89%8D%E4%BD%99%E9%A2%9D%EF%BC%9A%E3%80%90{{dqyy|urlencode}}%E3%80%91%E5%85%83%20---%20%E5%B7%B2%E6%B6%88%E8%B4%B9%E3%80%90{{yxf|urlencode}}%E3%80%91%E5%85%83",
              'headers': [],
              'cookies': []},
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}], 'failed_asserts': [], 'extract_variables': [{'name': '__log__', 're': '.+', 'from': 'content'}]}}]
