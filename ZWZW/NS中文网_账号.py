...
# 运行：python NS中文网_账号.py   （鉴权信息见下方内置配置；环境变量 NS211_ACCOUNT / NS211_DOMAIN / NS211_PROXY / NS211_DELAY / NS211_COOKIE_CACHE / NS211_FORCE_LOGIN / NS211_TLS_IMPERSONATE 优先）
# 多账户：环境变量或内置配置里填多个，用 换行 / @@ / && 分隔，也可直接写 Python 列表
# 账号/密码鉴权：登录后把会话 Cookie（及登录步骤提取的变量）缓存到脚本同目录 <名>.cookie_cache.json，下次直接复用、过期自动重新登录（NS211_COOKIE_CACHE 可改路径）。若站点不靠 Cookie 维持登录（纯 token / 每次需重登），设 NS211_FORCE_LOGIN=1 强制每次账号密码登录、不读写缓存。
...
BUILTIN_ACCOUNT = ""  # 推荐：账号#密码（# 也可写 :）；多组换行/@@/&&分隔，如 "u1#p1@@u2#p2"；也可用 NS211_ACCOUNT 环境变量
BUILTIN_USER = "xxx"  # 登录账号；多账户用换行/@@/&&分隔，或写列表
BUILTIN_PASS = "xxx"  # 登录密码，与账号一一对应（只填一个则多账号共用）
BUILTIN_FORCE_LOGIN = False  # 设为 True 强制每次账号密码登录（不读写 cookie 缓存），应对不靠 cookie 维持登录的站点；也可用 NS211_FORCE_LOGIN=1 开启
...
BUILTIN_AUTO_INSTALL = True  # 依赖缺失时自动 pip install（如 curl_cffi）；设 NS211_AUTO_INSTALL=0 关闭
BUILTIN_STRICT_EMPTY = True  # 汇总变量全空时判失败（防"未登录却报成功"）；站点本就不返回数据可设 NS211_STRICT_EMPTY=0
# =================================

BASE = "https://www.ns211.com"
...
ENTRIES = [{'request': {'method': 'POST',
              ...
              'headers': [{'name': ':authority', 'value': 'www.ns211.com'},
                          {'name': ':method', 'value': 'POST'},
                          {'name': ':path', 'value': '/wp-admin/admin-ajax.php'},
                          ...
                          {'name': 'dnt', 'value': '1'},
                          {'name': 'origin', 'value': 'https://www.ns211.com'},
                          {'name': 'referer', 'value': 'https://www.ns211.com/'},
                          {'name': 'sec-ch-ua', 'value': '"Chromium";v="122", "Not(A:Brand";v="24", "Microsoft Edge";v="122"'},
                          {'name': 'sec-ch-ua-mobile', 'value': '?0'},
                          {'name': 'sec-ch-ua-platform', 'value': '"Windows"'},
                          {'name': 'sec-fetch-dest', 'value': 'empty'},
                          {'name': 'sec-fetch-mode', 'value': 'cors'},
                          {'name': 'sec-fetch-site', 'value': 'same-origin'},
                          {'name': 'user-agent', 'value': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36 Edg/122.0.0.0'},
                          {'name': 'x-requested-with', 'value': 'XMLHttpRequest'}],
              'cookies': [],
              'data': 'action=user_login&username={{user|urlencode}}&password={{password|urlencode}}',
              'mimeType': 'application/x-www-form-urlencoded; charset=UTF-8'},
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}], 'failed_asserts': [], 'extract_variables': [{'name': 'msg', 're': '"msg":"(.*?)"', 'from': 'content'}]},
  'login': True},
 {'comment': 'Unicode转换',
  'request': {'method': 'POST', 'url': 'api://util/unicode', 'headers': [], 'cookies': [], 'data': 'html_unescape=false&content={{msg}}'},
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}, {'re': '"状态": "200"', 'from': 'content'}],
           'failed_asserts': [],
           'extract_variables': [{'name': '__log__', 're': '"转换后": "(.*)"', 'from': 'content'}]}},
 {'request': {'method': 'GET',
              'url': 'https://www.ns211.com/',
              'headers': [{'name': ':authority', 'value': 'www.ns211.com'},
 ...
 {'request': {'method': 'POST',
              ...
              'headers': [{'name': ':authority', 'value': 'www.ns211.com'},
                          ...
                          {'name': 'referer', 'value': 'https://www.ns211.com/7404.html'},
                          {'name': 'sec-ch-ua', 'value': '"Chromium";v="122", "Not(A:Brand";v="24", "Microsoft Edge";v="122"'},
                          {'name': 'sec-ch-ua-mobile', 'value': '?0'},
                          {'name': 'sec-ch-ua-platform', 'value': '"Windows"'},
                          {'name': 'sec-fetch-dest', 'value': 'empty'},
  ...
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}], 'failed_asserts': [], 'extract_variables': [{'name': 'msg', 're': '"msg":"(.*?)"', 'from': 'content'}]}},
 {'request': {'method': 'GET',
              'url': 'https://www.ns211.com/7404.html',
              'headers': [{'name': ':authority', 'value': 'www.ns211.com'},
                          {'name': ':method', 'value': 'GET'},
                          {'name': ':path', 'value': '/7404.html'},
                          {'name': ':scheme', 'value': 'https'},
                          {'name': 'accept', 'value': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7'},
                          {'name': 'accept-encoding', 'value': 'gzip, deflate, br'},
                          {'name': 'accept-language', 'value': 'zh-CN,zh;q=0.9'},
                          ...
                          {'name': 'user-agent', 'value': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36 Edg/122.0.0.0'}],
              'cookies': []},
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}], 'failed_asserts': [], 'extract_variables': [{'name': 'rmb', 're': '余额：\\d+.\\d+', 'from': 'content'}]}},
 {'comment': 'Unicode转换',
  'request': {'method': 'POST', 'url': 'api://util/unicode', 'headers': [], 'cookies': [], 'data': 'html_unescape=false&content={{msg}}。当前{{rmb}}'},
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}, {'re': '"状态": "200"', 'from': 'content'}],
           'failed_asserts': [{'re': '请登录后签到', 'from': 'content'}],
           'extract_variables': [{'name': '__log__', 're': '"转换后": "(.*)"', 'from': 'content'}]}}]
