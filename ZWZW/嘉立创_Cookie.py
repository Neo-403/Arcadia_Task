...
# 手写（遵循 签到脚本生成标准.md；oshwhub.com 嘉立创开源 每日签到；Cookie 鉴权）
# 运行：python 嘉立创_Cookie.py   （Cookie 见下方内置配置；环境变量 JLC_COOKIE / JLC_PROXY / JLC_DELAY / JLC_TLS_IMPERSONATE 优先）
# 多账户：Cookie 用换行 / @@ / && 分隔，也可直接写 Python 列表；本脚本为 Cookie 模式，不含账号密码
# Cookie 鉴权（自带 Cookie，不本地存储；Cookie 失效请更新 BUILTIN_COOKIE / JLC_COOKIE）
...
# （如 SITE_TAG="JLC" 时，Cookie 用 JLC_COOKIE，代理 JLC_PROXY）。
SITE_TAG = "JLC"
...
# 多账户另一种写法（列表）：BUILTIN_COOKIE = ["ck1", "ck2"]；旧写法仍兼容
BUILTIN_COOKIE = ""  # 会话 Cookie；多账户用换行/@@/&&分隔，或写列表 ["ck1","ck2"]
...
BUILTIN_AUTO_INSTALL = True  # 依赖缺失时自动 pip install（如 curl_cffi）；设 JLC_AUTO_INSTALL=0 关闭
BUILTIN_STRICT_EMPTY = True  # 汇总变量全空时判失败（防"未登录却报成功"）；站点本就不返回数据可设 JLC_STRICT_EMPTY=0
# =================================
...
ENTRIES = [{'comment': '每日签到',
  'request': {'method': 'POST',
              'url': 'https://oshwhub.com/api/users/signIn',
              'headers': [{'name': 'Cookie', 'value': '{{cookie}}'},
                          {'name': 'Content-Type', 'value': 'application/json;charset=UTF-8'},
                          {'name': 'Origin', 'value': 'https://oshwhub.com'},
                          {'name': 'Referer', 'value': 'https://oshwhub.com/sign_in'},
                          {'name': 'User-Agent', 'value': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/153.0.0.0 Safari/537.36 Edg/153.0.0.0'}],
              'cookies': [],
              'data': '{"_t":{{_t}}}'},
  'rule': {'success_asserts': [{'re': '"success":true', 'from': 'content'},
                               {'re': '"code":0', 'from': 'content'},
                               {'re': '"result":true', 'from': 'content'},
                               {'re': '已签到|已经签到|今日已签到|今天已签到|重复签到', 'from': 'content'}],
           'failed_asserts': [{'re': '请先登录|未登录|登录后再|凭证失效|登录失败', 'from': 'content'}],
           'extract_variables': []}},
 {'comment': '获取签到资料(含当前积分)',
  'request': {'method': 'GET',
              'url': 'https://oshwhub.com/api/users/getSignInProfile?_t={{_t}}',
              'headers': [{'name': 'Cookie', 'value': '{{cookie}}'},
                          {'name': 'Referer', 'value': 'https://oshwhub.com/sign_in'},
                          {'name': 'User-Agent', 'value': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/153.0.0.0 Safari/537.36 Edg/153.0.0.0'},
  ...
  'rule': {'success_asserts': [{'re': '"success":true', 'from': 'content'},
                               {'re': '"code":0', 'from': 'content'},
                               {'re': '"total_point"', 'from': 'content'}],
           'failed_asserts': [{'re': '请先登录|未登录|登录后再|凭证失效|登录失败', 'from': 'content'}],
           'extract_variables': [{'re': r'"total_point":(\d+)', 'from': 'content', 'name': 'total_point'},
                                 {'re': r'"week_signIn_days":(\d+)', 'from': 'content', 'name': 'week_days'},
                                 {'re': r'"month_signIn_days":(\d+)', 'from': 'content', 'name': 'month_days'}]}},
 {'comment': '汇总',
  'request': {'method': 'POST', 'url': 'api://util/unicode', 'headers': [], 'cookies': [],
              'data': 'content=嘉立创开源(oshwhub)每日签到成功，当前积分：{{total_point}}（本周已签 {{week_days}} 天 / 本月 {{month_days}} 天）'},
...
def run():
    ...
    for i, ck in enumerate(cookies):
        c = {"cookie": ck, "_t": str(int(time.time() * 1000))}
