...
# 运行：python 柚坛社区_账号.py   （鉴权信息见下方内置配置；环境变量 UOTAN_ACCOUNT / UOTAN_DOMAIN / UOTAN_PROXY / UOTAN_DELAY / UOTAN_COOKIE_CACHE / UOTAN_FORCE_LOGIN / UOTAN_TLS_IMPERSONATE 优先）
# 多账户：环境变量或内置配置里填多个，用 换行 / @@ / && 分隔，也可直接写 Python 列表
# 账号/密码鉴权：登录后把会话 Cookie（及登录步骤提取的变量）缓存到脚本同目录 <名>.cookie_cache.json，下次直接复用、过期自动重新登录（UOTAN_COOKIE_CACHE 可改路径）。若站点不靠 Cookie 维持登录（纯 token / 每次需重登），设 UOTAN_FORCE_LOGIN=1 强制每次账号密码登录、不读写缓存。
...
BUILTIN_ACCOUNT = ""  # 推荐：账号#密码（# 也可写 :）；多组换行/@@/&&分隔，如 "u1#p1@@u2#p2"；也可用 UOTAN_ACCOUNT 环境变量
BUILTIN_USER = "xxx"  # 登录账号；多账户用换行/@@/&&分隔，或写列表
BUILTIN_PASS = "xxx"  # 登录密码，与账号一一对应（只填一个则多账号共用）
BUILTIN_FORCE_LOGIN = False  # 设为 True 强制每次账号密码登录（不读写 cookie 缓存），应对不靠 cookie 维持登录的站点；也可用 UOTAN_FORCE_LOGIN=1 开启
...
BUILTIN_AUTO_INSTALL = True  # 依赖缺失时自动 pip install（如 curl_cffi）；设 UOTAN_AUTO_INSTALL=0 关闭
BUILTIN_STRICT_EMPTY = True  # 汇总变量全空时判失败（防"未登录却报成功"）；站点本就不返回数据可设 UOTAN_STRICT_EMPTY=0
# =================================

BASE = "https://www.uotan.cn"
...
ENTRIES = [{'request': {'method': 'GET',
              'url': 'https://www.uotan.cn/login/',
              'headers': [{'name': 'Host', 'value': 'www.uotan.cn'},
                          {'name': 'Connection', 'value': 'keep-alive'},
                          {'name': 'Pragma', 'value': 'no-cache'},
                          {'name': 'Cache-Control', 'value': 'no-cache'},
                          {'name': 'sec-ch-ua', 'value': '"Google Chrome";v="103", "Chromium";v="103", "Not=A?Brand";v="24"'},
                          {'name': 'sec-ch-ua-mobile', 'value': '?0'},
                          {'name': 'sec-ch-ua-platform', 'value': '"Windows"'},
                          {'name': 'Upgrade-Insecure-Requests', 'value': '1'},
                          {'name': 'User-Agent', 'value': 'Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Safari/537.36'},
                          {'name': 'Accept', 'value': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7'},
              ...
              'data': '',
              'mimeType': ''},
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}], 'failed_asserts': [], 'extract_variables': [{'name': 'token', 're': 'data-csrf="(.+?)"', 'from': 'content'}]}},
 {'request': {'method': 'POST',
              'url': 'https://www.uotan.cn/login/login',
              'headers': [{'name': 'Host', 'value': 'www.uotan.cn'},
                          {'name': 'Connection', 'value': 'keep-alive'},
                          ...
                          {'name': 'sec-ch-ua-platform', 'value': '"Windows"'},
                          {'name': 'Upgrade-Insecure-Requests', 'value': '1'},
                          {'name': 'Origin', 'value': 'https://www.uotan.cn'},
                          ...
                          {'name': 'Sec-Fetch-User', 'value': '?1'},
                          {'name': 'Sec-Fetch-Dest', 'value': 'document'},
                          {'name': 'Referer', 'value': 'https://www.uotan.cn/login/'},
                          {'name': 'Accept-Encoding', 'value': 'gzip, deflate, br'},
                          {'name': 'Accept-Language', 'value': 'zh-CN,zh;q=0.9,en;q=0.8,en-GB;q=0.7,en-US;q=0.6'}],
              'cookies': [],
              'data': '_xfToken={{token|urlencode}}&login={{username|urlencode}}&password={{password|urlencode}}&remember=1&_xfRedirect=%2F',
              'mimeType': 'application/x-www-form-urlencoded'},
  'rule': {'success_asserts': [{'re': '303', 'from': 'status'}, {'re': '200', 'from': 'content'}], 'failed_asserts': [], 'extract_variables': []},
  'login': True},
 {'comment': '每日签到',
  'request': {'method': 'POST',
              'url': 'https://www.uotan.cn/mjc-credits/clock',
              'headers': [{'name': 'Host', 'value': 'www.uotan.cn'},
                          {'name': 'Connection', 'value': 'keep-alive'},
                          {'name': 'Cache-Control', 'value': 'max-age=0'},
                          {'name': 'sec-ch-ua', 'value': '"Google Chrome";v="103", "Chromium";v="103", "Not=A?Brand";v="24"'},
                          ...
                          {'name': 'Sec-Fetch-User', 'value': '?1'},
                          {'name': 'Sec-Fetch-Dest', 'value': 'document'},
                          {'name': 'Referer', 'value': 'https://www.uotan.cn'},
                          {'name': 'Accept-Encoding', 'value': 'gzip, deflate, br'},
                          {'name': 'Accept-Language', 'value': 'zh-CN,zh;q=0.9,en;q=0.8,en-GB;q=0.7,en-US;q=0.6'}],
              'cookies': [],
              'data': '_xfToken={{token|urlencode}}',
              'mimeType': 'application/x-www-form-urlencoded'},
  'rule': {'success_asserts': [{'re': '303', 'from': 'status'}, {'re': '200', 'from': 'content'}], 'failed_asserts': [], 'extract_variables': []}},
 {'request': {'method': 'GET',
              'url': 'https://www.uotan.cn/mjc-credits/',
  ...
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}],
           'failed_asserts': [],
           'extract_variables': [{'name': 'log_value', 're': '<dt>积分</dt>\\n\\t\\t\\t\\t<dd>\\n\\t\\t\\t\\t\\t(.+)\\n\\t\\t\\t\\t</dd>', 'from': 'content'}]}},
 {'comment': '生成日志',
  'request': {'method': 'POST', 'url': 'api://util/urldecode', 'headers': [], 'cookies': [], 'data': 'content=积分: {{log_value}}'},
