...
# 运行：python 七日杀中文网_账号.py   （鉴权信息见下方内置配置；环境变量 Q7RISHA_ACCOUNT / Q7RISHA_DOMAIN / Q7RISHA_PROXY / Q7RISHA_DELAY / Q7RISHA_COOKIE_CACHE / Q7RISHA_FORCE_LOGIN / Q7RISHA_TLS_IMPERSONATE 优先）
# 多账户：环境变量或内置配置里填多个，用 换行 / @@ / && 分隔，也可直接写 Python 列表
# 账号/密码鉴权：登录后把会话 Cookie（及登录步骤提取的变量）缓存到脚本同目录 <名>.cookie_cache.json，下次直接复用、过期自动重新登录（Q7RISHA_COOKIE_CACHE 可改路径）。若站点不靠 Cookie 维持登录（纯 token / 每次需重登），设 Q7RISHA_FORCE_LOGIN=1 强制每次账号密码登录、不读写缓存。
...
BUILTIN_ACCOUNT = ""  # 推荐：账号#密码（# 也可写 :）；多组换行/@@/&&分隔，如 "u1#p1@@u2#p2"；也可用 Q7RISHA_ACCOUNT 环境变量
BUILTIN_USER = "xxx"  # 登录账号；多账户用换行/@@/&&分隔，或写列表
BUILTIN_PASS = "xxx"  # 登录密码，与账号一一对应（只填一个则多账号共用）
BUILTIN_FORCE_LOGIN = False  # 设为 True 强制每次账号密码登录（不读写 cookie 缓存），应对不靠 cookie 维持登录的站点；也可用 Q7RISHA_FORCE_LOGIN=1 开启
...
BUILTIN_AUTO_INSTALL = True  # 依赖缺失时自动 pip install（如 curl_cffi）；设 Q7RISHA_AUTO_INSTALL=0 关闭
BUILTIN_STRICT_EMPTY = True  # 汇总变量全空时判失败（防"未登录却报成功"）；站点本就不返回数据可设 Q7RISHA_STRICT_EMPTY=0
# =================================

BASE = "https://www.7risha.com"
...
ENTRIES = [{'comment': '登录',
  'request': {'method': 'POST',
              'url': 'https://www.7risha.com/wp-content/themes/LightSNS/module/action/login.php',
              'headers': [{'name': 'Host', 'value': 'www.7risha.com'},
                          ...
                          {'name': 'Origin', 'value': 'https://www.7risha.com'},
                          {'name': 'Sec-Fetch-Site', 'value': 'same-origin'},
                          {'name': 'Sec-Fetch-Mode', 'value': 'cors'},
                          {'name': 'Sec-Fetch-Dest', 'value': 'empty'},
                          {'name': 'Referer', 'value': 'https://www.7risha.com/'},
  ...
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}, {'re': '登录成功！欢迎回来！', 'from': 'content'}], 'failed_asserts': [], 'extract_variables': []},
  'login': True},
 {'comment': '签到',
  'request': {'method': 'POST',
              'url': 'https://www.7risha.com/wp-content/themes/LightSNS/module/action/sign.php',
              'headers': [{'name': 'Host', 'value': 'www.7risha.com'},
                          {'name': 'Connection', 'value': 'keep-alive'},
                          {'name': 'sec-ch-ua', 'value': '"Chromium";v="122", "Not(A:Brand";v="24", "Google Chrome";v="122"'},
                          {'name': 'Accept', 'value': 'application/json, text/javascript, */*; q=0.01'},
              ...
              'mimeType': 'application/x-www-form-urlencoded'},
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}], 'failed_asserts': [], 'extract_variables': [{'name': 'msg', 're': '"msg":"(.*?)",', 'from': 'content'}]}},
 {'comment': '签到天数获取',
  'request': {'method': 'GET',
              'url': 'https://www.7risha.com/sign',
              'headers': [{'name': 'Host', 'value': 'www.7risha.com'},
                          {'name': 'Connection', 'value': 'keep-alive'},
  ...
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}],
           'failed_asserts': [],
           'extract_variables': [{'name': 'ljqd', 're': '<div class="jinsom-sign-page-all-days">累计签到<span>(\\d+)', 'from': 'content'}, {'name': 'id', 're': '"user_id":(\\d+)', 'from': 'content'}]}},
 {'comment': '每日奖励领取',
  'request': {'method': 'POST',
              'url': 'https://www.7risha.com/wp-content/themes/LightSNS/module/action/task.php',
              'headers': [{'name': 'Host', 'value': 'www.7risha.com'},
                          {'name': 'Connection', 'value': 'keep-alive'},
                          {'name': 'sec-ch-ua', 'value': '"Chromium";v="122", "Not(A:Brand";v="24", "Google Chrome";v="122"'},
                          {'name': 'Accept', 'value': '*/*'},
 ...
 {'comment': '每日奖励领取',
  'request': {'method': 'POST',
              ...
              'headers': [{'name': 'Host', 'value': 'www.7risha.com'},
                          ...
                          {'name': 'Accept-Language', 'value': 'zh-CN,zh;q=0.9'}],
              'cookies': [],
              'data': 'task_id=task009&type=day',
              'mimeType': 'application/x-www-form-urlencoded'},
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}], 'failed_asserts': [], 'extract_variables': []}},
 {'comment': '金币信息',
  'request': {'method': 'POST',
              'url': 'https://www.7risha.com/wp-content/themes/LightSNS/module/stencil/mywallet.php',
              'headers': [{'name': 'Host', 'value': 'www.7risha.com'},
                          {'name': 'Connection', 'value': 'keep-alive'},
                          {'name': 'sec-ch-ua', 'value': '"Chromium";v="122", "Not(A:Brand";v="24", "Google Chrome";v="122"'},
                          {'name': 'Accept', 'value': '*/*'},
  ...
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}],
           'failed_asserts': [],
           'extract_variables': [{'name': 'jb', 're': '<span class="jinsom-icon jinsom-jinbi"></span>[\\s|\\S]*?<i>(\\d+)', 'from': 'content'}]}},
 {'comment': 'Unicode转换',
  'request': {'method': 'POST', 'url': 'api://util/unicode', 'headers': [], 'cookies': [], 'data': 'html_unescape=false&content=签到状态：{{msg}}\\r\\n累计签到：{{ljqd}}天\\r\\n金币：{{jb}}'},
