...
# 运行：python 飞牛私有云社区_Cookie.py   （鉴权信息见下方内置配置；环境变量 FNNAS_COOKIE / FNNAS_PROXY / FNNAS_DELAY / FNNAS_TLS_IMPERSONATE 优先）
# 多账户：环境变量或内置配置里填多个，用 换行 / @@ / && 分隔，也可直接写 Python 列表
# Cookie 鉴权（自带 Cookie，不本地存储；Cookie 失效请更新 BUILTIN_COOKIE / FNNAS_COOKIE）
...
BUILTIN_AUTO_INSTALL = True  # 依赖缺失时自动 pip install（如 curl_cffi）；设 FNNAS_AUTO_INSTALL=0 关闭
BUILTIN_STRICT_EMPTY = True  # 汇总变量全空时判失败（防"未登录却报成功"）；站点本就不返回数据可设 FNNAS_STRICT_EMPTY=0
# =================================

BASE = "https://club.fnnas.com"
BUILTIN_ALLOW_CROSS = True   # 允许跨域请求(保留第三方授权/API域名步骤)：True/False；也可用 <SITE_TAG>_ALLOW_CROSS=1 开启
...
ENTRIES = [{'request': {'method': 'GET',
              ...
              'headers': [{'name': 'Host', 'value': 'club.fnnas.com'},
                          ...
                          {'name': 'Referer', 'value': 'https://club.fnnas.com/'},
              ...
              'data': '',
              'mimeType': ''},
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}], 'failed_asserts': [], 'extract_variables': [{'name': 'sign', 're': 'sign&sign=(.+)" class="btna', 'from': 'content'}]}},
 {'request': {'method': 'GET',
              'url': 'https://club.fnnas.com/plugin.php?id=zqlj_sign&sign={{sign|urlencode}}',
              'headers': [{'name': 'Host', 'value': 'club.fnnas.com'},
                          {'name': 'Connection', 'value': 'keep-alive'},
                          {'name': 'sec-ch-ua', 'value': '"Chromium";v="122", "Not(A:Brand";v="24", "Google Chrome";v="122"'},
                          {'name': 'sec-ch-ua-mobile', 'value': '?0'},
                          ...
                          {'name': 'Sec-Fetch-User', 'value': '?1'},
                          {'name': 'Sec-Fetch-Dest', 'value': 'document'},
                          {'name': 'Referer', 'value': 'https://club.fnnas.com/plugin.php?id=zqlj_sign'},
                          {'name': 'Accept-Encoding', 'value': 'gzip, deflate, br'},
                          {'name': 'Accept-Language', 'value': 'zh-CN,zh;q=0.9'},
                          {'name': 'Cookie', 'value': '{{cookie}}'}],
              'cookies': [],
 ...
 {'request': {'method': 'GET',
  ...
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}],
           'failed_asserts': [],
           'extract_variables': [{'name': 'fnb', 're': '飞牛币: </em>(\\d+)', 'from': 'content'},
                                 {'name': 'nz', 're': '牛值: </em>(\\d+)', 'from': 'content'},
                                 {'name': 'ts', 're': '登陆天数: </em>(\\d+)', 'from': 'content'},
                                 {'name': 'jf', 're': '积分: </em>(\\d+)', 'from': 'content'}]}},
 {'comment': 'Unicode转换',
  'request': {'method': 'POST', 'url': 'api://util/unicode', 'headers': [], 'cookies': [], 'data': 'html_unescape=false&content=飞牛币:{{fnb}} | 牛值:{{nz}} | 登录天数:{{ts}} | 积分:{{jf}}'},
