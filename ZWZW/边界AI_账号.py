...
# 运行：python 边界AI_账号.py   （鉴权信息见下方内置配置；环境变量 AI1FOO_ACCOUNT / AI1FOO_DOMAIN / AI1FOO_PROXY / AI1FOO_DELAY / AI1FOO_COOKIE_CACHE / AI1FOO_FORCE_LOGIN / AI1FOO_TLS_IMPERSONATE 优先）
# 多账户：环境变量或内置配置里填多个，用 换行 / @@ / && 分隔，也可直接写 Python 列表
# 账号/密码鉴权：登录后把会话 Cookie（及登录步骤提取的变量）缓存到脚本同目录 <名>.cookie_cache.json，下次直接复用、过期自动重新登录（AI1FOO_COOKIE_CACHE 可改路径）。若站点不靠 Cookie 维持登录（纯 token / 每次需重登），设 AI1FOO_FORCE_LOGIN=1 强制每次账号密码登录、不读写缓存。
...
BUILTIN_ACCOUNT = ""  # 推荐：账号#密码（# 也可写 :）；多组换行/@@/&&分隔，如 "u1#p1@@u2#p2"；也可用 AI1FOO_ACCOUNT 环境变量
BUILTIN_USER = "xxx"  # 登录账号；多账户用换行/@@/&&分隔，或写列表
BUILTIN_PASS = "xxx"  # 登录密码，与账号一一对应（只填一个则多账号共用）
BUILTIN_FORCE_LOGIN = False  # 设为 True 强制每次账号密码登录（不读写 cookie 缓存），应对不靠 cookie 维持登录的站点；也可用 AI1FOO_FORCE_LOGIN=1 开启
...
BUILTIN_AUTO_INSTALL = True  # 依赖缺失时自动 pip install（如 curl_cffi）；设 AI1FOO_AUTO_INSTALL=0 关闭
BUILTIN_STRICT_EMPTY = True  # 汇总变量全空时判失败（防"未登录却报成功"）；站点本就不返回数据可设 AI1FOO_STRICT_EMPTY=0
# =================================

BASE = "https://ai1foo.com"

ENTRIES = [{'comment': '账号密码登录',
  'request': {'method': 'POST',
              'url': 'https://ai1foo.com/api/v1/user/login',
              'headers': [{'name': 'Host', 'value': 'ai1foo.com'},
                          ...
                          {'name': 'app-name', 'value': 'bianjie'},
                          {'name': 'sec-ch-ua-platform', 'value': '"Windows"'},
                          {'name': 'Accept', 'value': '*/*'},
                          {'name': 'Origin', 'value': 'https://ai1foo.com'},
                          {'name': 'Sec-Fetch-Site', 'value': 'same-origin'},
                          {'name': 'Sec-Fetch-Mode', 'value': 'cors'},
                          {'name': 'Sec-Fetch-Dest', 'value': 'empty'},
                          {'name': 'Referer', 'value': 'https://ai1foo.com/login'},
                          {'name': 'Accept-Encoding', 'value': 'gzip, deflate, br'},
                          {'name': 'Accept-Language', 'value': 'zh-CN,zh;q=0.9'}],
              'cookies': [],
              'data': '{"user":"{{username}}","pass":"{{password}}"}',
              'mimeType': 'application/json'},
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}, {'re': 'ok', 'from': 'content'}],
           'failed_asserts': [{'re': '用户不存在或密码错误', 'from': 'content'}],
           'extract_variables': [{'name': 'yhm', 're': '"user":"(.*?)",', 'from': 'content'},
                                 {'name': 'token', 're': '"token":"(.*?)",', 'from': 'content'},
                                 {'name': 'access_token', 're': '"access_token":"(.*?)"', 'from': 'content'},
                                 {'name': 'appid', 're': '"appid":(\\d+)', 'from': 'content'},
                                 {'name': 'id', 're': '"id":(\\d+)', 'from': 'content'}]},
  'login': True},
 {'comment': '签到',
  'request': {'method': 'POST',
              'url': 'https://api.ai1foo.com/api/v2/user/signin/do',
              'headers': [{'name': 'Host', 'value': 'api.ai1foo.com'},
                          {'name': 'Connection', 'value': 'keep-alive'},
                          {'name': 'sec-ch-ua', 'value': '"Chromium";v="122", "Not(A:Brand";v="24", "Google Chrome";v="122"'},
                          {'name': 'sec-ch-ua-mobile', 'value': '?0'},
                          ...
                          {'name': 'Sec-Fetch-Mode', 'value': 'cors'},
                          {'name': 'Sec-Fetch-Dest', 'value': 'empty'},
                          {'name': 'Referer', 'value': 'https://ai1foo.com/signIn'},
                          {'name': 'Accept-Encoding', 'value': 'gzip, deflate, br'},
                          {'name': 'Accept-Language', 'value': 'zh-CN,zh;q=0.9'}],
              'cookies': [],
              'data': '{}',
              'mimeType': 'application/json'},
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}], 'failed_asserts': [], 'extract_variables': [{'name': 'lxqd', 're': '"cont_day":(\\d+)', 'from': 'content'}]}},
 {'comment': '积分',
  'request': {'method': 'POST',
              'url': 'https://api.ai1foo.com/api/v1/user/points',
              'headers': [{'name': 'Host', 'value': 'api.ai1foo.com'},
                          {'name': 'Connection', 'value': 'keep-alive'},
                          {'name': 'sec-ch-ua', 'value': '"Chromium";v="122", "Not(A:Brand";v="24", "Google Chrome";v="122"'},
                          {'name': 'sec-ch-ua-mobile', 'value': '?0'},
                          ...
                          {'name': 'Sec-Fetch-Mode', 'value': 'cors'},
                          {'name': 'Sec-Fetch-Dest', 'value': 'empty'},
                          {'name': 'Referer', 'value': 'https://ai1foo.com/console/pointslog'},
                          {'name': 'Accept-Encoding', 'value': 'gzip, deflate, br'},
                          {'name': 'Accept-Language', 'value': 'zh-CN,zh;q=0.9'}],
              'cookies': [],
              'data': '',
              'mimeType': ''},
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}], 'failed_asserts': [], 'extract_variables': [{'name': 'jf', 're': 'points":(\\d+)', 'from': 'content'}]}},
 {'comment': 'Unicode转换',
  'request': {'method': 'POST', 'url': 'api://util/unicode', 'headers': [], 'cookies': [], 'data': 'html_unescape=false&content=用户名：{{yhm}}，已连续签到{{lxqd}}天，累计积分：{{jf}}。'},
