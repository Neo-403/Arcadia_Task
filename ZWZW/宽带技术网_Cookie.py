...
# 运行：python 宽带技术网_Cookie.py   （鉴权信息见下方内置配置；环境变量 CHINADSL_COOKIE / CHINADSL_PROXY / CHINADSL_DELAY / CHINADSL_TLS_IMPERSONATE 优先）
# 多账户：环境变量或内置配置里填多个，用 换行 / @@ / && 分隔，也可直接写 Python 列表
# Cookie 鉴权（自带 Cookie，不本地存储；Cookie 失效请更新 BUILTIN_COOKIE / CHINADSL_COOKIE）
...
BUILTIN_AUTO_INSTALL = True  # 依赖缺失时自动 pip install（如 curl_cffi）；设 CHINADSL_AUTO_INSTALL=0 关闭
BUILTIN_STRICT_EMPTY = True  # 汇总变量全空时判失败（防"未登录却报成功"）；站点本就不返回数据可设 CHINADSL_STRICT_EMPTY=0
# =================================

BASE = "https://www.chinadsl.net"
...
ENTRIES = [{'comment': '登录验证',
  'request': {'method': 'GET',
              ...
              'headers': [{'name': 'Connection', 'value': 'keep-alive'},
                          ...
                          {'name': 'User-Agent', 'value': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/110.0.0.0 Safari/537.36'},
                          ...
                          {'name': 'Referer', 'value': 'https://www.chinadsl.net/home.php'},
                          {'name': 'Accept-Encoding', 'value': 'gzip, deflate'},
                          {'name': 'Accept-Language', 'value': 'zh-CN,zh;q=0.9,en;q=0.8'},
                          {'name': 'Cookie', 'value': '{{cookie}}'}],
              'cookies': []},
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}], 'failed_asserts': [{'re': '您需要先登录才能继续本操作', 'from': 'content'}], 'extract_variables': []}},
 {'comment': '领取签到任务',
  'request': {'method': 'GET',
              ...
              'headers': [{'name': 'Connection', 'value': 'keep-alive'},
                          ...
                          {'name': 'User-Agent', 'value': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/110.0.0.0 Safari/537.36'},
                          {'name': 'Accept', 'value': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9'},
                          {'name': 'Referer', 'value': 'https://www.chinadsl.net/home.php?mod=task&do=view&id=1'},
                          {'name': 'Accept-Encoding', 'value': 'gzip, deflate'},
                          {'name': 'Accept-Language', 'value': 'zh-CN,zh;q=0.9,en;q=0.8'}],
              'cookies': []},
  'rule': {'success_asserts': [{'re': '302', 'from': 'status'}, {'re': '200', 'from': 'status'}], 'failed_asserts': [], 'extract_variables': []}},
 {'comment': '完成签到任务',
  'request': {'method': 'GET',
  ...
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}],
           'failed_asserts': [],
           'extract_variables': [{'name': 'msg', 're': 'messagetext[\\w\\W]+<p>([一-龥0-9，！]+)<[\\w\\W]+alert_btnleft', 'from': 'content'}]}},
 {'comment': '积分页',
  'request': {'method': 'GET',
  ...
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}],
           'failed_asserts': [],
           'extract_variables': [{'name': 'ml', 're': '猫粮: </em>(\\d+)', 'from': 'content'},
                                 {'name': 'ww', 're': '威望: </em>(\\d+)', 'from': 'content'},
                                 {'name': 'gx', 're': '贡献: </em>(\\d+)', 'from': 'content'},
                                 {'name': 'tg', 're': '推广: </em>(\\d+)', 'from': 'content'},
                                 {'name': 'jf', 're': '积分: </em>(\\d+)', 'from': 'content'},
                                 {'name': 'nickName', 're': '访问我的空间">(.+?)<', 'from': 'content'}]}},
 {'comment': 'Unicode转换',
  'request': {'method': 'POST',
