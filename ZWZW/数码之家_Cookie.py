...
# 运行：python 数码之家_Cookie.py   （鉴权信息见下方内置配置；环境变量 MYDIGIT_COOKIE / MYDIGIT_PROXY / MYDIGIT_DELAY / MYDIGIT_TLS_IMPERSONATE 优先）
# 多账户：环境变量或内置配置里填多个，用 换行 / @@ / && 分隔，也可直接写 Python 列表
# Cookie 鉴权（自带 Cookie，不本地存储；Cookie 失效请更新 BUILTIN_COOKIE / MYDIGIT_COOKIE）
...
BUILTIN_AUTO_INSTALL = True  # 依赖缺失时自动 pip install（如 curl_cffi）；设 MYDIGIT_AUTO_INSTALL=0 关闭
BUILTIN_STRICT_EMPTY = True  # 汇总变量全空时判失败（防"未登录却报成功"）；站点本就不返回数据可设 MYDIGIT_STRICT_EMPTY=0
# =================================

BASE = "https://www.mydigit.cn"
...
ENTRIES = [{'request': {'method': 'GET',
              'url': 'https://www.mydigit.cn/plugin.php?id=k_misign:sign',
              'headers': [{'name': 'Host', 'value': 'www.mydigit.cn'},
                          {'name': 'Pragma', 'value': 'no-cache'},
                          {'name': 'Cache-Control', 'value': 'no-cache'},
                          {'name': 'User-Agent', 'value': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.114 Safari/537.36 Edg/91.0.864.54'},
                          {'name': 'Referer', 'value': 'https://www.mydigit.cn/home.php'},
  ...
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}],
           'failed_asserts': [{'re': '登录', 'from': 'content'}, {'re': '系统错误', 'from': 'content'}],
           'extract_variables': [{'re': 'formhash=(.*?)">', 'from': 'content', 'name': 'formhash'}]}},
 {'request': {'method': 'GET',
              'url': 'https://www.mydigit.cn/plugin.php?id=k_misign:sign&operation=qiandao&format=empty&inajax=1&ajaxtarget=JD_sign&formhash={{formhash|urlencode}}',
              'headers': [{'name': 'Host', 'value': 'www.mydigit.cn'},
                          {'name': 'Pragma', 'value': 'no-cache'},
                          {'name': 'Cache-Control', 'value': 'no-cache'},
                          {'name': 'User-Agent', 'value': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.114 Safari/537.36 Edg/91.0.864.54'},
                          {'name': 'Referer', 'value': 'https://www.mydigit.cn/plugin.php?id=k_misign:sign'},
                          {'name': 'Accept-Encoding', 'value': 'gzip, deflate, br'},
                          {'name': 'Accept-Language', 'value': 'zh-CN,zh;q=0.9,en;q=0.8,en-GB;q=0.7,en-US;q=0.6'},
                          {'name': 'Upgrade-Insecure-Requests', 'value': '1'}],
              'cookies': [],
              'data': '',
              'mimeType': ''},
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}], 'failed_asserts': [{'re': '系统错误', 'from': 'content'}], 'extract_variables': []}},
 {'request': {'method': 'GET',
              'url': 'https://www.mydigit.cn/plugin.php?id=k_misign%3Asign',
              'headers': [{'name': 'Host', 'value': 'www.mydigit.cn'},
                          {'name': 'User-Agent', 'value': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.114 Safari/537.36 Edg/91.0.864.54'},
              ...
              'mimeType': ''},
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}],
           'failed_asserts': [{'re': 'id="JD_sign"', 'from': 'content'}],
           'extract_variables': [{'re': 'id="lxreward" value="(\\d+)"', 'from': 'content', 'name': 'log_value'}]}},
 {'comment': '生成日志',
  'request': {'method': 'GET', 'url': 'api://util/string/replace?r=text&t=%C2%A0&p=%C2%A0&s=%E7%A7%AF%E5%88%86%3A%C2%A0%20{{log_value|urlencode}}', 'headers': [], 'cookies': []},
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}], 'failed_asserts': [], 'extract_variables': [{'re': '.+', 'from': 'content', 'name': '__log__'}]}}]
