...
# 运行：python 木木云盘搜_账号.py   （鉴权信息见下方内置配置；环境变量 ACLINK_ACCOUNT / ACLINK_DOMAIN / ACLINK_PROXY / ACLINK_DELAY / ACLINK_COOKIE_CACHE / ACLINK_FORCE_LOGIN / ACLINK_TLS_IMPERSONATE 优先）
# 多账户：账号用 <SITE_TAG>_ACCOUNT 一次填完（"账号#密码"，多组换行 / @@ / && 分隔）；Cookie 用 换行 / @@ / && 分隔，也可直接写 Python 列表
# 账号/密码鉴权：登录后把会话 Cookie（及登录步骤提取的变量）缓存到脚本同目录 <名>.cookie_cache.json，下次直接复用、过期自动重新登录（ACLINK_COOKIE_CACHE 可改路径）。若站点不靠 Cookie 维持登录（纯 token / 每次需重登），设 ACLINK_FORCE_LOGIN=1 强制每次账号密码登录、不读写缓存。
...
BUILTIN_ACCOUNT = ""  # 推荐：账号#密码（# 也可写 :）；多组换行/@@/&&分隔，如 "u1#p1@@u2#p2"；也可用 ACLINK_ACCOUNT 环境变量
BUILTIN_USER = ""  # 旧写法：只填账号（BUILTIN_ACCOUNT 为空时才生效）；多账户换行/@@/&&分隔，或写列表
BUILTIN_PASS = ""  # 旧写法：登录密码，与账号一一对应（只填一个则多账号共用）
BUILTIN_FORCE_LOGIN = False  # 设为 True 强制每次账号密码登录（不读写 cookie 缓存），应对不靠 cookie 维持登录的站点；也可用 ACLINK_FORCE_LOGIN=1 开启
...
BUILTIN_AUTO_INSTALL = True  # 依赖缺失时自动 pip install（如 curl_cffi）；设 ACLINK_AUTO_INSTALL=0 关闭
BUILTIN_STRICT_EMPTY = True  # 汇总变量全空时判失败（防"未登录却报成功"）；站点本就不返回数据可设 ACLINK_STRICT_EMPTY=0
# =================================

BASE = "https://api.aclink.top"
...
ENTRIES = [{'request': {'method': 'POST',
              ...
              'headers': [{'name': ':authority', 'value': 'api.aclink.top'},
                          {'name': ':method', 'value': 'POST'},
                          {'name': ':path', 'value': '/user/login'},
                          ...
                          {'name': 'dnt', 'value': '1'},
                          {'name': 'origin', 'value': 'https://aclink.top'},
                          {'name': 'pragma', 'value': 'no-cache'},
                          {'name': 'priority', 'value': 'u=1, i'},
                          {'name': 'referer', 'value': 'https://aclink.top/'},
                          {'name': 'sec-ch-ua', 'value': '"Chromium";v="152", "Not?A_Brand";v="24", "Microsoft Edge";v="152"'},
                          {'name': 'sec-ch-ua-mobile', 'value': '?0'},
                          {'name': 'sec-ch-ua-platform', 'value': '"Windows"'},
                          {'name': 'sec-fetch-dest', 'value': 'empty'},
                          {'name': 'sec-fetch-mode', 'value': 'cors'},
                          {'name': 'sec-fetch-site', 'value': 'same-site'},
                          {'name': 'user-agent', 'value': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/152.0.0.0 Safari/537.36 Edg/152.0.0.0'}],
              'cookies': [],
 ...
 {'request': {'method': 'GET',
              'url': 'https://api.aclink.top/user',
              'headers': [{'name': ':authority', 'value': 'api.aclink.top'},
                          {'name': ':method', 'value': 'GET'},
                          {'name': ':path', 'value': '/user'},
                          {'name': ':scheme', 'value': 'https'},
                          {'name': 'accept', 'value': 'application/json, text/plain, */*'},
                          {'name': 'accept-encoding', 'value': 'gzip, deflate, br, zstd'},
                          {'name': 'accept-language', 'value': 'zh-CN,zh;q=0.9'},
  ...
  'rule': {'success_asserts': [{'re': '200', 'from': 'status'}],
           'failed_asserts': [],
           'extract_variables': [{'name': 'email', 're': '"Email":"(.+)","LoginIP', 'from': 'content'}, {'name': 'sou', 're': '","Sou":(.+),"VipTime', 'from': 'content'}]}},
 {'comment': 'Unicode转换',
  'request': {'method': 'POST', 'url': 'api://util/unicode', 'headers': [], 'cookies': [], 'data': 'html_unescape=false&content=Email：{{email}}\\r\\n签到：{{msg}}\\r\\n积分：{{sou}}'},
